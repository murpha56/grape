...
def ge_eaSimpleWithElitism(population, toolbox, cxpb, mutpb, ngen, elite_size,
    ...
    for ind in population:
        if not ind.fitness.valid:
            if fitfunc == 'MSE':
                ind.fitness.values = toolbox.evaluate_MSE(ind, points_train)
            elif fitfunc == 'LS':
                ind.fitness.values = toolbox.evaluate_LS(ind, points_train)
            elif fitfunc == 'Corr':
                ind.fitness.values = toolbox.evaluate_Corr(ind, points_train)
            else:
                print("Not valid Fitness Function")
